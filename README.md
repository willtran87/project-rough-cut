...
- A 14-second Delivery Chain that rewards linking smart plays—course progress, risky ball recoveries, bunker baits, optional paperwork, contact breaks, and capped Evidence Denied recoveries—without changing survival difficulty. Simultaneous rewards now play as a compact ordered card sequence with a visible next-count and queued value; a bounded five-card presentation queue aggregates only visual overflow while preserving every scoring event in the ledger.
