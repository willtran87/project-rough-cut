...
- **Objective markers:** The key and sprinkler now use image-generated world props, distance labels, projected interaction rings, and explicit keyboard/gamepad/touch in-reach states; the clipboard, shed, and drain use the same range language. Obstacle-aware gold/teal ground reflectors, fairway-edge stakes, physical zone signs, and a left/ahead/right field bearing make the active route readable from the first-person scene. Production levels should support configurable marker strength and more diegetic navigation through yardage books, course signs, landmarks, or recovered maintenance maps.
- **Spatial model:** The slice now has authored collision, projected boundaries, line-of-sight occlusion, hiding terrain, multiple traversable routes, obstacle-aware Joe navigation, and first-person route guidance generated from the same collision model. Nearby solid props reveal their footprint and name before contact; contact feedback points to the blocker and recommends a physically valid escape direction. The full mini-map folds away after the opening orientation window and returns only during Listening Focus. Production work still needs larger cover volumes, richer authored path choices, and multi-area navigation.
