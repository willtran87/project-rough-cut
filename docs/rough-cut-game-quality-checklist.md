...
- **Objective markers:** The key and sprinkler now use image-generated world props, distance labels, projected interaction rings, and explicit keyboard/gamepad/touch in-reach states; the clipboard, shed, and drain use the same range language. Production levels should support configurable marker strength and more diegetic navigation through yardage books, course signs, landmarks, or recovered maintenance maps.
- **Spatial model:** The slice now has authored collision, projected boundaries, line-of-sight occlusion, hiding terrain, multiple traversable routes, obstacle-aware Joe navigation, and a mini-map that draws the true collision footprint of every blocker—including non-rendered tunnel sides. Contact feedback names and points to the blocker, recommends a physically valid escape direction, and Listening Focus reveals nearby solid footprints. Production work still needs larger cover volumes, richer authored path choices, and multi-area navigation.
- **Boundary contract:** The maintained-course limits are explicitly outlined on the mini-map and report named edge contact instead of silently clamping movement. World art, collision radius, use radius, and mini-map geometry now share one coordinate source so a decorative prop cannot quietly disagree with traversal.
