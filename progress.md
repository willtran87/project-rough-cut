...
- Separated the two roles without changing pacing or rules. The bottom rail, orange ground corridor, and course-map header keep ownership of the safe route; the compact mower-amber caption now reads `JOE KEEPS MOVING` and derives its bearing from Joe's real position. The tested moment reads `JOE KEEPS MOVING — RIGHT` alongside `CLEAR LEFT`.
- Added `footing_entry_signal_role_separation`, diagnostics for the Joe-bearing/safe-route split, and static verification. Readiness is now 55/55; 48 assets and 87 deterministic scenarios remain clean.
- Official and fresh parallel responsive replays passed with no browser errors. The exact decision remained legible at 2560x1600 and 800x600, with 60 estimated FPS and 3.9/3.7 ms p95 render work respectively. No ImageGen asset was needed: this was a tactical information-hierarchy repair over the generated course art. Evidence: `qa/footing-threat-route-separation-polish-2026-08-10.md`.

## Tension Director signal-convergence polish (2026-08-09)

- Audited 581 recent runtime states and replayed the quiet opening cadence through ordinary input. One ambient Tension Director beat could present three versions of the same information at once: a grass/fog/bird omen caption, a directional mower-echo caption, and the bottom `THE DISTANT MOWER CHANGES PITCH` warning. The repetition made a strong suspense beat feel like HUD noise.
- Added a presentation-only convergence pass for captions created by the same ambient beat. The directional mower echo now wins over its paired environmental omen, while unrelated danger captions remain eligible for the second field slot and a standalone omen—such as Silent Stalk's grass warning—remains unchanged.
- Exposed the candidate, visible, and suppressed keys plus the optional bottom-warning relationship in `render_game_to_text`. Added `tension_director_signal_convergence`, bringing settled readiness to 54/54; the release verifier still passes all 87 deterministic scenarios and 48 referenced assets with no warnings.
- A natural real-time replay reached the second quiet beat without state injection. The 2560x1600 capture shows one directional `MOWER ECHOES ACROSS THE COURSE — AHEAD` card and the complementary bottom escalation rail, with the redundant grass omen removed. Responsive inspection at 2560x1600, 1280x720, and 800x600 remained contained with no browser errors. No ImageGen asset was needed because the generated environment already reads strongly; this pass improves how its suspense cues are edited. Evidence: `qa/tension-director-signal-convergence-polish-2026-08-09.md`.

## Unfiled Change Request HUD-truth polish (2026-08-09)

- Replayed the live Night Order route through Audit Bell and around the Water Hazard to secure `CR-017` through ordinary movement and Interact input. The generated clipboard, paper burst, world caption, Joe reaction, Field Log handoff, and bank-versus-appeal message all remained coherent, and the previously requested checked glyph rendered cleanly.
- The persistent HUD then exposed one semantic ambiguity: `CR ✓ +650` looked like an already-awarded score even though the document remains unfiled until a successful exit and can still be sacrificed during a close chase.
- Centralized the four Change Request HUD states and changed the secured state to `CR ✓ BANK +650`. Available, appeal-ready, and appealed states retain `CR ◇ +650`, `CR ! APPEAL READY`, and `CR X APPEALED`, so the persistent token now communicates the same risk/reward contract as the pickup message and result scoring.
- Added `unfiled_change_request_hud_truth`, bringing settled readiness to 53/53. The post-change official replay held 60 estimated FPS at 2.59 ms average, 3.7 ms p95, 30 ms observed max, one long frame, and no browser errors; all 48 runtime assets and 87 deterministic scenarios remain clean.
- Responsive inspection of the earned secured state passed at 2560x1600, 1280x720, and 800x600 without clipping or overlap. No ImageGen asset was needed because the existing generated Change Request art already passed direct visual inspection; this pass corrects persistent state communication. Evidence: `qa/unfiled-change-request-hud-truth-polish-2026-08-09.md`.

## Clean Break trail-evidence priority polish (2026-08-09)

- Extended the missed Audit Bell replay through the entire deferred-feedback lifecycle instead of stopping at three seconds. The lesson correctly paused through False Retreat, but its first resumed frame overlapped `TRAIL CHAIN ×3 // CHANGE SURFACE`: optional retry coaching occupied the bottom rail while the top banner and original message were telling the player to make an immediate route-changing survival decision.
- Added `trail_evidence` to the missed Clean Break critical-focus policy, matching the already-correct earned-reward policy. Miss coaching now pauses without consuming its remaining display time while Joe is actively following physical prints; the authoritative `CHANGE SURFACE` warning and cross-fairway/cut-turf instruction retain the decision lane until the evidence chain clears.
- A safe full-chain replay held the optional lesson at 2.63 seconds through the live trail investigation, resumed it at 2.62 seconds after 18.33 seconds, deferred it again at 0.43 seconds for the earned Evidence Denied Delivery result, resumed with 0.35 seconds, and expired at 23.17 seconds. Field Log remained the active world/map route throughout, and the ordinary `TRAIL COLD` instruction returned after the lesson expired.
- Added `clean_break_miss_trail_evidence_priority`, bringing settled readiness to 52/52. The full lifecycle run held 60 estimated FPS at 4.19 ms average, 6.6 ms p95, 29.6 ms observed max, one long frame, and no browser errors. The official activation regression passed 52/52 at 3.4 ms average and 3.6 ms p95 with all 48 runtime assets settled.
- Visual inspection of the exact clean resume frame passed at 2560x1600, 1280x720, and 800x600 with no clipping, HUD overlap, or loss of the hedge, fog, route thread, map, lantern, and background layers. No ImageGen asset was needed because this pass corrects temporal HUD ownership over the existing generated art. Evidence: `qa/clean-break-trail-evidence-priority-polish-2026-08-09.md`.

## Clean Break miss-recovery handoff polish (2026-08-09)

- Replayed Audit Bell through a recoverable failure: the player crossed Hedge Tunnel, left its authored cover before Joe's final check, survived signal expiry, and continued toward Field Log. The miss state, orange result hierarchy, `CLEAN 0/3` badge, Joe search, and same-frame route handoff were all correct, but the lesson still spoke as if the finished check were in progress: `stay behind HEDGE TUNNEL until Joe's check ends`.
- Rewrote all three miss outcomes as compact cause -> next-attempt rule -> recovery handoff. Leaving cover now says it happened before Joe's final check and teaches `HOLD COVER NEXT TIME`; movement teaches `HOLD STILL NEXT TIME`; an incomplete hold says `HOLD NEVER SET` and repeats the 0.9-second requirement. Every variant names the actual next mandatory station instead of a generic `next station`, so the bottom rail, first-person route, course map, and objective row agree immediately.
- Preserved the survival-first presentation contract. False Retreat and Recommit still preempt the optional lesson, its 2.65-second timer remains paused at 2.63 seconds, Field Log navigation stays active, and no score, detection, Joe pathing, signal timing, collision, or objective rule changed.
- Added `clean_break_miss_recovery_handoff_copy`, bringing settled readiness to 51/51. The recoverable-miss run held 60 estimated FPS at 5.13 ms average, 6.9 ms p95, 34.8 ms observed max, one long frame, and no browser errors. The official activation regression passed 51/51 at 3.43 ms average and 3.8 ms p95 with all 48 runtime assets settled.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600 with no clipping, protected-HUD overlap, or loss of the generated hedge, station, fog, map, and layered course art. No ImageGen asset was needed because this pass repairs consequence language and recovery hierarchy over the existing authored assets. Evidence: `qa/clean-break-miss-recovery-handoff-polish-2026-08-09.md`.

## Clean Break same-frame resolution polish (2026-08-09)

- Extended the native Audit Bell replay through Hedge Tunnel arrival, 0.9-second quiet hold, signal mask, verification expiry, Clean Break reward, False Retreat, and resumed Field Log route. The underlying loop was correct, but the exact success frame combined a new `CLEAN BREAK` result with one cached frame of `HOLD QUIET 0m` on the map and `HOLD QUIET` in first-person diagnostics.
- Signal expiry now retires the old distraction owner, refreshes the navigation guide, and invalidates the minimap in the same update. The reward frame therefore names Field Log in the world, map, and diagnostics immediately instead of contradicting a completed challenge. A retained diagnostic handoff records Audit Bell → Field Log, refresh ownership, cache invalidation, and presentation-only gameplay scope.
- The same screenshot exposed a reward-tone mismatch: earned Clean Break used the orange border reserved for danger and failed consequences. The success rail now uses a dark-green ground, mint outer frame, restrained inner highlight, and pale mint text; failed and ordinary consequence rails keep their existing danger trim. This gives optional mastery a clearer, more satisfying payoff without adding screen density.
- Added `field_signal_resolution_handoff_coherence`, bringing settled readiness to 50/50. The native resolution frame reported Field Log at 331.94 meters, `CLEAN 1/3`, +169 Delivery, Joe sweeping the Audit Bell area, and the Clean Break lesson visible with its full 2.65-second duration. False Retreat then deferred that lesson at 2.63 seconds without disturbing the Field Log route.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600. The final adaptive run held 60 estimated FPS at 2.94 ms average, 3.7 ms p95, 29.7 ms observed max, one long frame, and no browser errors. The official activation regression passed 50/50 with all 48 runtime assets settled. No ImageGen asset was needed because this pass improves the temporal and tonal presentation of existing generated course art. Evidence: `qa/clean-break-same-frame-resolution-polish-2026-08-09.md`.

## Breakaway destination and hold-ownership polish (2026-08-09)

- Replayed the complete Audit Bell commitment and collision-aware escape to Hedge Tunnel. `BREAK SIGNAL` correctly explained why the player had to move, but it also replaced the physical destination in the immediate route card, first-person target, rear bearing, and course map. The player could commit to `BREAK TO HEDGE TUNNEL` and then receive a less useful generic destination under pressure.
- Split tactical reason from physical destination without changing gameplay. The bottom consequence rail retains `BREAK SIGNAL`, while every navigation channel now preserves the exact authored cover promised before activation: Hedge Tunnel, Service Cart, or Range Cart. The route card, first-person target, rear bearing, course map, commitment preview, and text diagnostics share that destination until arrival.
- Native traversal reached Hedge Tunnel in 131 frames with 4.12 seconds left on the Audit Bell check. The exact arrival frame exposed one last duplicate: the dedicated `MASK YOUR ROUTE` panel briefly overlapped a second result/handoff rail. Reached cover now gives the dedicated hold panel exclusive presentation ownership while preserving the underlying message and handoff timers; leaving cover can still restore relevant consequence copy.
- Added `field_signal_breakaway_destination_continuity` and `field_breakaway_hold_panel_ownership`, bringing settled readiness to 49/49. The official client passed at 60 estimated FPS with 4.09 ms average, 5.2 ms p95, and 33 ms observed max render work. The adaptive arrival run held 60 FPS at 4.9 ms p95 with no browser errors.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600 for activation and exact cover arrival. The release verifier passes 87 deterministic scenarios and 48 referenced assets with no warnings. No ImageGen asset was needed because this pass improves navigation and presentation hierarchy around existing generated station, hedge, map, and environment art. Evidence: `qa/field-signal-breakaway-destination-continuity-polish-2026-08-09.md`.

## Ready-interaction visual-focus polish (2026-08-09)

- A native Audit Bell approach exposed a hierarchy gap after the action itself was already clear: an unrelated distant Shed Key marker, the non-active Audit Thatch bypass card, and its amber ground arrow still competed with the generated bell, live pressure read, and exact commitment rail. Activating the bell replaced the key marker but allowed the optional thatch advice to compete with the mandatory Hedge Tunnel breakaway.
- Added one presentation-only commitment-focus contract. A ready Interact owner now defers distant navigation cards while retaining physical prop art, collision truth, the persistent course map, and any overlapping action that is also genuinely in reach. Optional footing advice now yields as one unit—card and ground route—during either a ready action or an immediate collision/footing/breakaway route. An actually active footing hazard always remains authoritative.
- Native pre-commit replay reached Audit Bell at 11 meters with Joe patrolling at 102 meters. The bell art, coherent low-pressure ring, local label, action rail, and map remained; the Shed Key card, Audit Thatch card, and optional bypass route were absent. Enter still advanced field checks to 1/3, started the existing 6.3-second bell signal, and transferred all immediate navigation to the 59-meter Hedge Tunnel breakaway without restoring the thatch overlay.
- Added `ready_interaction_visual_focus` and `web/test-actions/ready-interaction-visual-focus.json`, bringing settled readiness to 47/47. Official pre-commit validation held 60 estimated FPS at 3.32 ms average, 4.9 ms p95, and 29.8 ms observed max; the breakaway branch held 60 FPS at 3.13 ms average, 3.6 ms p95, and 32.4 ms max. Both reported no browser errors.
- True browser-viewport inspection passed at 2560x1600, 1280x720, and 800x600. The stage stayed centered and contained, the decision lane stayed anchored, and no optional overlay returned during responsive resizing. The release verifier passes 87 deterministic action scenarios and 48 referenced assets with no warnings. No ImageGen asset was needed because the improvement protects the hierarchy around the existing generated station, hazard, and landmark art. Evidence: `qa/ready-interaction-visual-focus-polish-2026-08-09.md`.

## Loud-station pressure-transition stability polish (2026-08-09)

- Followed the golf-ball diversion replay at two-frame resolution and found that Joe repeatedly touched the 78-meter watch/high boundary. The pressure surface was accurate at each sample, but without transition memory its outline, fill, label, frame, and pulse could visibly chatter when route steering moved Joe across the threshold for only a fraction of a second.
- Added one run-local, target-specific safety-first transition model. A new target snapshots current truth; non-critical escalation confirms after 0.18 seconds; recovery confirms after 0.38 seconds; changing raw pressure clears an unfinished transition; leaving the interaction clears its memory; and `CRITICAL` bypasses stabilization immediately. Only presentation is held—the live Joe distance, mode, sightline, alert, AI, detection, action, route, and consequence remain unchanged.
- Native sampling proved a 0.13-second touch of `HIGH RISK` at 78 meters stayed visibly `WATCH JOE`, then cleared cleanly. Sustained search later promoted to `HIGH RISK` at 75 meters after the 0.18-second confirmation; a patrol recovery held watch through 0.38 seconds before settling low; a fresh search promoted low to watch after 0.18 seconds; and 23-meter proximity became `CRITICAL` immediately.
- A second native branch committed Audit Bell from stable `HIGH RISK` at 72 meters. Enter still advanced field checks to 1/3, started the existing 6.2-second Audit Bell signal, reset the completed station's transition memory, and retained the Hedge Tunnel breakaway. Added `loud_station_pressure_transition_stability`, bringing settled readiness to 46/46.
- The official 1,284-frame browser replay passed 46/46 at 60 estimated FPS with 3.17 ms average, 4.0 ms p95, and 31.2 ms observed max render work and no browser errors. Visual inspection passed at 2560x1600, 1280x720, and 800x600. The static verifier passes 86 deterministic scenarios and 48 referenced assets. No ImageGen asset was needed because this pass stabilizes the presentation of the existing generated station family. Evidence: `qa/loud-station-pressure-transition-stability-polish-2026-08-09.md`.

## Loud-station pressure-surface and dialogue-clearance polish (2026-08-09)

- The official low-pressure Audit Bell frame exposed a mixed visual signal: the new mint outline still inherited the generic amber in-reach fill. A real golf-ball diversion then exposed a second semantic issue: Joe entering `investigate` at 119 meters immediately read `HIGH RISK`, even though the player had just spent a resource to redirect him successfully.
- Added one coherent tier-specific ring treatment. `LOW PRESSURE`, `WATCH JOE`, `HIGH RISK`, and `CRITICAL` now share their outline, translucent ground wash, line weight, and reduced-motion-safe pulse profile. Low pressure is calmer and less dominant; urgency progressively strengthens through watch, high, and critical. Generic key, valve, exit, request, and practice rings retain their existing treatment.
- Refined current-pressure truth so a long-range investigation or search reads `WATCH JOE`; it escalates to `HIGH RISK` only when Joe closes to 78 meters, approaches within 54 meters regardless of mode, or combines elevated alert with meaningful range. Chase, line of sight, and 24-meter proximity remain critical. This preserves the value of smart golf-ball counterplay without pretending that an active sweep is fully safe.
- The same high-pressure replay found Joe's optional subtitle covering the generated bell and interaction footprint. Ready world interactions now relocate Joe's bark from the grounded center lane to the open top-center lane without expiring it, while the station art, ring, pressure label, and two-line consequence keep the physical decision lane.
- Added `in_reach_joe_bark_clearance`, bringing settled readiness to 45/45, plus `web/test-actions/loud-station-pressure-surface.json`. The official 2,510-frame browser replay held 60 estimated FPS at 2.74 ms average, 3.8 ms p95, and 35.5 ms observed max with no browser errors. Native golf-ball counterplay verified `WATCH JOE` at 119 meters, `HIGH RISK` at 78 meters, preserved Interact, and completed Audit Bell 1/3 with its 6.2-second signal. Visual inspection passed at 2560x1600, 1280x720, and 800x600. No ImageGen asset was needed because this pass protects and visually unifies the existing generated Night Order station art. Evidence: `qa/loud-station-pressure-surface-polish-2026-08-09.md`.

## Loud-station commitment-pressure polish (2026-08-09)

- Extended the real-input Night Order replay through Audit Bell, Water Hazard, Clubhouse Crossing, and Field Log. At the Field Log commitment point Joe was already searching at 45 meters, yet the ready station used the same calm presentation as an Audit Bell commitment with Joe patrolling at 102 meters. The exact authored consequence was present, but the immediate wait-or-commit pressure was not.
- Added a presentation-only four-tier Joe-pressure read derived from his current mode, distance, line of sight, and alert: `LOW PRESSURE`, `WATCH JOE`, `HIGH RISK`, or `CRITICAL`. The selected tier colors the generated station's ground ring, adds one compact local label above its physical art, and accents the existing bottom action frame. It never removes Interact or changes signal time, Joe AI, detection, scoring, survival, routes, or station geometry.
- Official low-pressure validation reached Audit Bell with Joe patrolling at 101.97 meters and displayed `LOW PRESSURE // JOE 102m`. The deep high-pressure replay reached Field Log at 9.73 meters with Joe searching at 45 meters and displayed `HIGH RISK // JOE 45m`; Enter still advanced the ledger to 2/3, started the existing 5.5-second signal, and preserved the Service Cart breakaway.
- Added `loud_station_commitment_pressure_read`, bringing settled readiness to 44/44. The official client held 60 estimated FPS at 3.93 ms average, 5.3 ms p95, and 30.2 ms observed max with no browser errors. Visual inspection passed at 2560x1600, 1280x720, and 800x600; the release verifier still passes all 85 deterministic action scenarios and 48 referenced assets. No ImageGen asset was needed because this pass strengthens decision hierarchy around the existing generated station family. Evidence: `qa/loud-station-commitment-pressure-polish-2026-08-09.md`.

## In-reach interaction scorecard-clearance polish (2026-08-09)

- Extended the real-input audit beyond Audit Bell, through trail pressure, optional-ball counterplay, the Water Hazard, and the Clubhouse Crossing to Field Log. The run reached the generated kiosk at 9.73 meters with the exact 5.6-second loud-signal and Service Cart consequence visible, but a newly banked Risk Premium card occupied the same center-world lane and covered the kiosk, its planted base, and much of its interaction ring.
- Added one presentation-only Risk Premium placement contract. The scorecard keeps its dramatic center lane ordinarily, but moves to the open top-center lane while any ready in-reach world object owns Interact. The award remains visible and expires on its original timer; interaction, score, risk, Joe, and objective mechanics are unchanged.
- The replay now shows the full Field Log art, shadow, use ring, prompt, consequence, map, and relocated `CONTACT BROKEN // RISK PREMIUM +166` card together. Enter still stamps Field Log, advances to 2/3, starts the existing 5.5-second verification signal, and routes toward Service Cart.
- Added `in_reach_interaction_scorecard_clearance`, bringing settled readiness to 43/43. The official browser replay held 60 estimated FPS at 3.11 ms average, 4.2 ms p95, and 27.6 ms observed max with no browser errors. Visual inspection passed at 2560x1600, 1280x720, and 800x600; the static verifier still passes all 85 deterministic action scenarios and 48 referenced assets. No ImageGen asset was needed because this pass protects the existing generated Field Log art. Evidence: `qa/in-reach-interaction-scorecard-clearance-polish-2026-08-09.md`.

## Clean Break reward-resume continuity polish (2026-08-09)

- Audited the completed Audit Bell escape and found that the earned `CLEAN BREAK — Joe verified the station, not your route.` lesson could expire unseen while False Retreat and physical trail pursuit correctly owned the bottom rail.
- The success lesson now queues beneath urgent survival, interaction, footing, collision, and predator-tactic presentation owners. Its remaining display time pauses while deferred, then resumes in full once the danger owner yields; the ordinary Delivery scorecard may coexist without suppressing the lesson.
- A native-input replay completed the real Audit Bell Clean Break, followed the authored Field Log counter-route, and proved the timer held at 2.63 seconds across two deferred samples before returning visibly with the same 2.63 seconds. After expiry, Field Log remained selected at 318.56 meters, `CLEAN 1/3` persisted, and the run retained a 169-point Delivery bonus.
- Added `clean_break_reward_resume_continuity`, bringing settled readiness to 42/42. The static verifier passes all 85 deterministic action scenarios and 48 referenced runtime assets. Visual inspection passed at 2560x1600, 1280x720, and 800x600; the official web-game client also passed 42/42 with no browser-error artifact. No ImageGen asset was needed because this pass repairs temporal feedback over the existing generated course art. Evidence: `qa/clean-break-reward-resume-continuity-polish-2026-08-09.md`.

## Clean Break map-state continuity polish (2026-08-09)

- Followed the successful Audit Bell hold into the persistent map and found a cross-channel contradiction: the new world panel correctly showed `SIGNAL MASKED`, while the map continued to present the reached Hedge Tunnel as `HOLD QUIET 8m`. The cover was no longer a destination, but one channel still treated it like one.
- Added a shared reached-cover map presentation. Before arrival the map preserves ordinary `SAFE BREAK` navigation; during the hold it switches to `HOLD STILL` plus live 0.9-second progress; after completion it switches to mint `MASKED // STAY HIDDEN`. The target remains available to route geometry, but stale distance copy is suppressed.
- Direct screenshots exposed one cached-frame lag at the exact hold-to-masked boundary. Added a critical Clean Break cache key so route, hold, masked, and signal-ended phase changes invalidate the 120–160ms minimap cache immediately without increasing ordinary map render frequency.
- Added `clean_break_map_hold_continuity`, bringing settled readiness to 41/41. A live native-input replay reached Hedge Tunnel, built the hold, masked the signal, awarded the existing Clean Break, and returned the map to Field Log with no browser errors. Visual inspection passed at 2560x1600, 1280x720, and 800x600; the official client also reached the unmodified Audit Bell commitment state and passed 41/41. The static verifier still passes all 85 action scenarios. No ImageGen asset was needed because this pass aligns gameplay communication over the existing generated course art. Evidence: `qa/clean-break-map-state-continuity-polish-2026-08-09.md`.

## Clean Break quiet-hold presentation polish (2026-08-09)

- Replayed the complete Audit Bell activation and successful breakaway loop. The mechanics were sound, but reached-cover feedback still used the generic world-marker glyph lane, leaving oversized detached `||` and `OK` symbols above a separate instruction card at the moment the player needed the clearest stillness read.
- Replaced that reached-cover marker with one screen-stable tactical panel. It now carries `MASK YOUR ROUTE` or `SIGNAL MASKED`, the live 0.9-second hold bar, Joe's remaining verification time, the named-cover instruction, and the completion state as one grounded hierarchy. The ordinary `SAFE BREAK` world marker remains unchanged until the exact authored cover is reached.
- Added `clean_break_hold_presentation`, bringing settled readiness to 40/40. The contract checks cover gating, half-progress and completed states, timer copy, named-cover continuity, canvas containment, mint success tone, and the explicit presentation-only gameplay contract; the static release verifier guards the same implementation markers and still passes all 85 deterministic action scenarios.
- A live native-input replay reached Hedge Tunnel, built the hold, completed `SIGNAL MASKED`, and preserved the existing Clean Break reward and next-objective handoff. Visual inspection passed at 2560x1600, 1280x720, and 800x600 with no detached debug glyphs or protected-HUD overlap. No ImageGen asset was needed because the pass preserves the generated hedge and field-station art while repairing its gameplay presentation. Evidence: `qa/clean-break-quiet-hold-presentation-polish-2026-08-09.md`.

## Loud field-check commitment preview polish (2026-08-09)

- Replayed the complete opening-to-Audit-Bell sequence and found that the in-reach action rail named the input and verb but dropped the consequence at the exact commitment point. The player had to remember from earlier briefing copy that the check would summon Joe and then discover the named escape cover only after pressing Interact.
- Added one presentation-only commitment preview to every mandatory station. The authoritative action owner now exposes the exact existing signal duration, states that Joe will verify it, and names the already-authored breakaway cover before activation: Hedge Tunnel for Audit Bell, Service Cart for Field Log, and Range Cart for Release Review.
- Expanded only those three high-risk action rails into a contained two-line panel. Keyboard, gamepad, and touch keep their live action binding on the first line; consequence and counterplay remain on the second. All ordinary interactions retain their original one-line geometry.
- Added `field_action_commitment_preview`, bringing settled readiness to 39/39, plus static release guards, text diagnostics, and `web/test-actions/field-action-commitment-preview.json`. The release verifier now covers 85 deterministic action scenarios.
- The required official client reached the Audit Bell through unmodified gameplay, rendered the 6.4-second warning and Hedge Tunnel answer, passed 39/39, and held 60 estimated FPS at 3.21 ms average / 4.7 ms p95 with no browser errors. The complete activation and breakaway chain remained intact. Visual inspection passed at 2560x1600, 1280x720, 800x600, and touch-sized 844x390.
- No ImageGen asset was needed because this pass makes the consequence of the existing generated stations legible before commitment. Evidence: `qa/loud-field-check-commitment-preview-polish-2026-08-09.md`.

## Compact Survival Briefing readability polish (2026-08-09)

- Replaced the short-viewport version of the desktop dossier with a dedicated compact hierarchy instead of scaling every card and keycap into unreadable miniature copy. The compact layout keeps the assignment context, three core decisions, two input-aware control lines, and one clear start action in larger horizontal rows while standard and high-resolution screens retain the full visual dossier.
- Added keyboard, gamepad, and touch-specific compact wording without changing the existing premise, objectives, input bindings, tutorial dismissal, world context, Reduced Camera Motion behavior, or First Steps handoff. Existing generated key, ball, and valve art remains inside the new decision rows.
- Added `compact_survival_briefing_readability`, bringing the settled gameplay readiness audit to 38/38, plus a static release guard, text diagnostics for layout/density/font floor, and `web/test-actions/compact-survival-briefing.json`. The release verifier now covers 84 deterministic action scenarios.
- The official client settled all assets, dismissed the briefing, moved 19 meters, passed 38/38, and held 60 estimated FPS at 4.3 ms average / 9 ms p95 render work with no browser errors. Responsive keyboard and touch validation passed at 2560x1600, 1280x720, 800x600, and 844x390 with exact containment, input-specific copy, working start controls, and no horizontal overflow.
- No ImageGen asset was needed because this pass improves the legibility and hierarchy of existing generated art. Evidence: `qa/compact-survival-briefing-readability-polish-2026-08-09.md`.

## Rear-view immediate-route camera handoff polish (2026-08-09)

- Reproduced a high-pressure camera contradiction: holding Rear during grounds-cart recovery reprojected the body-relative forward clearance lane onto the ground behind the player, where it appeared to point right while the course map still said `CLEAR LEFT`. Active slow-footing escape shared the same risk.
- Added a bounded camera handoff. Collision and footing world routes fade before the rear turn completes; the map retains the exact local decision, and the existing rear panel names the input-aware release action, body-relative direction, target, and distance. Returning forward restores the same route owner and physical geometry immediately.
- Added `rear_view_immediate_route_handoff`, bringing readiness to 37/37, plus static release guards and detailed text diagnostics for route alpha, deferral, map retention, and body-relative movement.
- The official collision regression held 60 estimated FPS at 2.43 ms average / 4.2 ms p95. Native collision and Irrigation Mud replays both verified forward → rear → forward continuity with no browser errors; the final states passed 37/37 at 60 FPS.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600 with exact viewport containment. No ImageGen asset was needed because this pass protects the spatial truth of the existing generated gate, cart, and footing art. Evidence: `qa/rear-view-immediate-route-camera-handoff-polish-2026-08-09.md`.

## Pause immediate-route action continuity polish (2026-08-09)

- Pausing during the opening grounds-cart recovery exposed one remaining hierarchy mismatch: the frozen map correctly said `CLEAR LEFT // GROUNDS CART CLEAR 7m`, but the pause modal reverted to the distant `FOLLOW LANTERNS // AUDIT BELL 125m` objective.
- Added one shared immediate-action presentation for collision clearance and slow-footing escape. The live dossier, pause modal, paused Settings context, and text diagnostics now repeat the same local direction, landmark, and distance until the route clears while retaining the preserved objective as the active file.
- Preserved mandatory signal breakaway, interaction rejection, filing, simulation freeze, route planning, collision, movement, Joe AI, input, scoring, and timing. Resuming retires no valid route, and ordinary long-horizon guidance returns automatically after clearance.
- Added `pause_immediate_route_action_continuity`, bringing readiness to 36/36, a static verifier guard, and `web/test-actions/pause-immediate-route-continuity.json`.
- Official collision, resume, and Irrigation Mud pause replays passed with modal/map agreement and no browser errors. The responsive run settled 48/48 assets, held 60 estimated FPS at 3.74 ms average / 6.9 ms p95, and passed at 2560x1600, 1280x720, 800x600, and paused Settings.
- No ImageGen asset was needed because this pass preserves and clarifies the existing generated cart, terrain, and course art. Evidence: `qa/pause-immediate-route-action-continuity-polish-2026-08-09.md`.

## Immediate-route world-marker ownership polish (2026-08-09)

- Replayed the 1,298-frame Audit Bell-to-Water Hazard route and found one remaining first-person hierarchy conflict: Irrigation Mud correctly owned `CLEAR LEFT` and a grounded `MUD EXIT`, but the unrelated optional `SHED KEY // 57m` plaque remained in the same left lane. Two destinations appeared to support one urgent instruction.
- Added one shared immediate-navigation marker owner for collision clearance, active slow-footing escape, and mandatory station breakaway. While any of those short-horizon routes is active, distant key, valve, exit, and mandatory-objective plaques yield completely; generated physical props, exact use footprints, collision, AI, objectives, map state, and interactions remain unchanged.
- Preserved the actionable boundary: an interactable inside its authoritative radius overrides suppression and retains its normal ready/ownership presentation. When the local route clears, ordinary selected and alternate marker emphasis returns automatically.
- Added `immediate_route_world_marker_ownership`, bringing readiness to 35/35, plus a static release guard and `web/test-actions/immediate-route-marker-handoff.json`. The release verifier now covers 82 deterministic action scenarios.
- The native active-mud replay retained only the amber local route and `MUD EXIT`; text state reported the Shed Key and Field Log plaques as `long_horizon_marker_deferred` with zero marker alpha while physical art and use footprints remained. It held 60 estimated FPS at 2.71 ms average, 4.7 ms p95, and 30.4 ms observed max with one long frame.
- The added real-input handoff moved left to `(-24, 223)`, reached fairway at full pace, retired the immediate owner, restored Field Log as the selected route, and restored Shed Key at alternate-route emphasis. That run held 60 FPS at 2.49 ms average, 3.5 ms p95, and 30.7 ms observed max with no browser errors.
- Exhaustive owner regression covered the other two paths. Grounds-cart clearance deferred Shed Key and Audit Bell under `collision_recovery:collision-clear:service-cart` at 2.30 ms average / 4.1 ms p95; Audit Bell breakaway deferred Shed Key and Field Log under `mandatory_noise_breakaway:field-signal-break-audit-bell` at 3.54 ms average / 4.7 ms p95. Both remained 35/35 at 60 FPS with no browser errors.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600 with exact viewport containment and no panel overlap. No new ImageGen asset was needed because the refinement preserves and clarifies the existing generated physical props rather than adding another object. Evidence: `qa/immediate-route-world-marker-ownership-polish-2026-08-09.md`.

## Status Request spatial-consequence polish (2026-08-09)

- Followed the acknowledged rough-grid consequence beyond its newly named Joe Attention handoff and found that its location still disappeared physically once the decision card closed. The map named the choice, but the first-person scene did not show where the shared grid ended or how far the player had escaped it.
- Added one presentation-only spatial model shared by investigation and follow-up search. Acknowledgment projects a broad mint dashed grid; escalation projects a tighter danger-orange solid sector with a crosshair. Both remain centered on the existing AI target and use the existing rough-versus-precise location radii.
- Added truthful inside/outside and distance-to-edge language to the world marker and course-map header. When the target falls behind the fixed forward camera, a compact rear-edge cue above the action rail says which area is behind, whether the player is still inside, and how many meters remain; it does not claim an input or displace collision, hazard, objective, or interaction ownership.
- Exposed the active spatial consequence and rear-edge cue through text diagnostics. Added `status_request_spatial_consequence_continuity`, bringing readiness to 34/34, and added static release guards for both shapes, the shared search handoff, placement, and presentation-only gameplay contract.
- Direct visual inspection passed at 2560x1600, 1280x720, and 800x600 for acknowledged inside/outside states plus the escalated sector. The world footprint, rear cue, map, objectives, and bottom action rail remained contained and readable with no browser errors.
- The official gameplay replay settled all 48 assets, passed readiness 34/34, and sustained 60 estimated FPS at 2.48 ms average render work, 4.5 ms p95, and 28.3 ms observed max with one long render frame. No new ImageGen asset was needed: this refinement deliberately projects gameplay truth over the existing generated course art. Evidence: `qa/status-request-spatial-consequence-polish-2026-08-09.md`.

## Status Request consequence-continuity polish (2026-08-09)

- Replayed the complete Status Request choice and found that its decision card was strong but its consequence became generic as soon as Joe took over: `VERIFYING DISTURBANCE` during travel and `SEARCHING LAST SIGNAL` during the follow-up sweep erased whether the player shared a rough grid or triggered a precise escalation.
- Added one presentation-only evidence handoff. Acknowledgment now remains a mint `ROUGH STATUS GRID`; escalation remains a danger-orange `ESCALATED SECTOR`; the same source, target, color, counterplay, and timer flow from investigation into Joe's existing search.
- Joe Attention now reports `VERIFYING` and then `SWEEPING` the chosen location type. Joe's grounded world label compresses the same state to `CHECKING/SWEEPING ROUGH GRID` or `CHECKING/SWEEPING YOUR SECTOR` without changing dialogue or cover suppression.
- Added inspectable investigation/follow-up state, `status_request_consequence_continuity`, and static release guards. Readiness is now 33/33; request timing, location precision, Delivery score, Joe AI, detection, movement, objectives, and input are unchanged.
- A 2,683-frame official keyboard route sustained 60 estimated FPS through four zones at 2.61ms average, 3.7ms p95, and 28ms maximum render time with one long frame and no browser errors. It also verified that active Crosswind Cover truthfully defers the request.
- A temporary visual-only hook invoked the production escalation resolver and normal Joe transition, then was removed. Direct inspection passed at 2560x1600, 1280x720, and 800x600 with the fitted `SWEEPING ESCALATED SECTOR // 5.5s` read fully contained in the existing panel. Evidence: `qa/status-request-consequence-continuity-polish-2026-08-09.md`.

## Clean Break miss-feedback and retry-loop polish (2026-08-09)

- Audited the long Audit Bell-to-Water Hazard route and found an engagement-loop asymmetry: a successful station mask received a complete reward, score, badge, and result path, while a failed Clean Break was only recorded internally. The live HUD stayed empty, so the player received no durable mastery goal or reason-specific lesson.
- Added outcome coaching for all three misses: leaving named cover early, moving or making noise at expiry, and failing to complete the 0.9-second hold. The copy names the exact correction and explicitly confirms that the next station can still score; score, station completion, survival, Joe search, and objective progression remain unchanged.
- The first attempt now reveals a compact `CLEAN 0/3` badge. A miss uses restrained burnt amber, the first success promotes it to mint, and a perfect 3/3 retains gold. The existing objective row keeps priority and no new permanent HUD row was added.
- Closed the presentation hierarchy during native verification: hazards and predator tactics correctly displaced the lesson, but originally allowed its timer to expire. Miss coaching now queues beneath exclusive survival/action states, pauses while deferred, outranks routine trail reminders and optional dialogue, then resumes in the existing bottom rail once the player reaches ordinary field state.
- Added `fieldSignalBreakawayMissPresentation()`, explicit deferral diagnostics, active/last feedback state, the `clean_break_miss_feedback_continuity` readiness contract, and `web/test-actions/field-signal-miss-feedback.json`. Readiness is now 32/32 and the release verifier covers 81 deterministic action scenarios.
- The final official route recorded `left_named_cover_before_expiry`, deferred through Irrigation Mud and False Retreat, reached fairway at `(-24, 223)`, and displayed the exact Hedge Tunnel correction with 2.05 seconds remaining. Joe stayed in search at 119m; the next Field Log route remained active. Rendering held 60 estimated FPS with 3.13ms average, 5.9ms p95, 39.4ms observed max, one long render frame, and no browser errors.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600 with exact document containment. The muted badge and resumed lesson stayed inside their established panels without obscuring the course, map, objective route, or generated art. No new ImageGen asset was needed for this feedback-only pass. Evidence: `qa/clean-break-miss-feedback-polish-2026-08-09.md`.

## Short collision-recovery first-person route polish (2026-08-09)

- Replayed the opening grounds-cart contact and found a final map-dependence gap: collision recovery correctly owned navigation for another 6.77 meters, but the ordinary world ribbon samples first at nine meters. The first-person scene therefore showed no route geometry after the contact card retired even though the map still said `BEAR LEFT`.
- Added a dedicated player-origin clearance corridor for bounded collision recovery. A restrained amber dashed lane, ordered chevrons, ground ring, diamond, and landmark-specific `GROUNDS CART CLEAR` marker now occupy the near field until the safe waypoint is reached; physical props still render over the route and remain visually solid.
- The same immediate owner now controls the course-map header and a short player-to-clearance segment. Slow-footing corridors yield if collision recovery is active, preventing two local escape routes from competing. Once clear, the local cue retires and the preserved objective replans normally.
- Exposed the route's visibility, player origin, segment count, marker, instruction, obstacle, phase, and handoff rule through `render_game_to_text()`. Added `collision_recovery_world_route_continuity`, bringing readiness to 31/31, plus a static release-verifier guard.
- The official fixture ended beside the authored grounds cart at `(11, 36)` with `CLEAR LEFT`, a visible one-segment world route, grounded marker, matching map instruction, and no browser errors. It held 60 estimated FPS with 4.7ms render p95. A further native 48-frame left move reached `(-8, 36)`, retired recovery, restored Audit Bell as the ordinary route owner, and returned 11 reflectors plus ten route-thread segments at 60 FPS.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600 with exact document containment, solid cart art, readable map/world agreement, and no panel overlap. No additional generated asset was needed because this polish deliberately grounds navigation beneath the existing ImageGen cart and course art. Evidence: `qa/short-collision-recovery-first-person-route-polish-2026-08-09.md`.

## Slow-footing first-person escape corridor polish (2026-08-09)

- Followed the settled Irrigation Mud route after the immediate-owner pass and found that the correct `CLEAR LEFT` decision still depended too heavily on text and the map. The world bypass began at a pre-entry point already behind the camera; filtering out offscreen endpoints reduced the route to no visible geometry in the original replay.
- Rebuilt the local bypass presentation as a player-origin first-person corridor. Intersecting route segments are clipped at the safe course edge instead of discarded, a restrained edge bridge represents the wide offscreen turn, directional chevrons preserve travel order, and a grounded material-specific exit ring and diamond make the destination readable against the generated course art.
- Closed the full handoff discovered during real movement validation: after the player clears a patch, that same nearby bypass now stays suppressed while behind or alongside them, so it cannot compete with the restored objective route. Entering the patch again or genuinely backtracking toward its entry restores the guidance.
- Added `footing_escape_world_route_continuity` and `footing_escape_post_clear_handoff`, bringing readiness to 30/30. Text diagnostics expose player-origin ingress, visible segments, offscreen continuation, edge bridge, exit marker, and the post-clear decision; the static release verifier guards both contracts.
- The final official deep-route replay reached `(22, 223)` in Irrigation Mud at 54% pace with one continuous three-segment escape corridor, a visible edge bridge, a grounded Mud Exit marker, Field Log preserved, and its conflicting objective reflectors suppressed. It passed 30/30 at an estimated 60 FPS with 2.78ms average, 4.0ms p95, and 31.3ms observed max render time.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600 with exact document containment and no panel overlap. A native 180-frame left move reached fairway at `(-24, 223)`, removed the immediate owner and local world route, preserved Field Log at 204.08m, and restored 11 reflectors plus ten objective-route segments. No browser errors or asset failures occurred. Evidence: `qa/slow-footing-first-person-escape-corridor-polish-2026-08-09.md`.

## Slow-footing immediate-route ownership polish (2026-08-09)

- Audited the settled Water Hazard route and found competing navigation instructions while the player was already slowed in Irrigation Mud: the consequence rail said `CLEAR LEFT`, while the persistent map, objective bearing, and mint Field Log route still directed the player toward the distant objective on the right.
- Added one latched short-horizon route owner for the active footing patch. Entry selects the collision-clear bypass once and preserves that side until the player exits, preventing Joe movement from flipping the recommendation mid-crossing. The local owner now controls the bearing, next waypoint, course-map header, orange map route, and exit marker while the mint objective ribbon, reflectors, and caption yield.
- The long-horizon objective remains intact throughout the crossing. Clearing the footing removes the override and replans the same objective from the player's actual exit position; no objective progress, Joe behavior, hazard geometry, or movement cost changed.
- Exposed the route owner, target, clearance, preserved objective, handoff, and `one_immediate_route_owner_short_horizon_before_long_horizon` rule through `render_game_to_text()`. Added `footing_escape_navigation_ownership`, bringing readiness to 28/28, plus a static release-verifier guard.
- An official deep-route replay entered Irrigation Mud at 54% pace with `CLEAR LEFT // MUD EXIT 27m`, zero objective reflectors, zero objective route segments, and the Field Log still preserved as the active objective. A native 180-frame left move then reached fairway at full pace, removed the override, and restored 11 reflectors plus ten Field Log route segments. No browser errors or asset failures occurred.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600 with exact document containment and no panel overlap. The active escape remained readable at every size; the cleared desktop view restored the mint route naturally. The responsive run held an estimated 60 FPS at 2.68ms active average, 4.4ms p95, and 31.8ms observed max render time. Evidence: `qa/slow-footing-immediate-route-ownership-polish-2026-08-09.md`.
- Final release verification passed 48 runtime assets, 80 deterministic action scenarios, zero source masters, zero unreferenced runtime images, and no warnings or failures.

## Collision-feedback identity ownership polish (2026-08-09)

- Audited the settled opening route with the official gameplay fixture and found a contradictory contact read: while the grounds cart visibly blocked the player and its grounded collision card was still completing its release echo, the large state banner said `SOUTH GATE // SOMETHING KNOCKS BACK`. The matching directional threat caption could also surface when that banner yielded, presenting two different collision identities for one moment.
- Added one authoritative collision-to-banner relationship covering active physical rejection and the bounded release echo. During either phase, the grounded card for the actual blocker owns immediate contact identity; unrelated state banners keep their authored timers but do not draw, and any matching threat-caption representation yields with them. Once the contact presentation clears, ordinary state/caption presentation resumes without changing Joe, movement, collision geometry, or set-piece timing.
- Exposed the owner, phase, deferred banner, matching-caption visibility, and `one_physical_contact_one_visible_identity` rule through `render_game_to_text()`. Added `collision_feedback_identity_ownership`, bringing readiness to 27/27, plus a static release-verifier guard.
- The official fixture replay ended on the real `service-cart` release echo: its collision owner was active, the unrelated South Gate banner reported `visible: false / deferredBy: collision_release_echo`, its matching caption produced zero visible cards, and no browser errors occurred. The run held 60 estimated FPS at 2.49ms average, 5.2ms p95, and 28.5ms observed max render time.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600. Each document matched its viewport exactly, the canvas remained centered and contained, the large cart retained its authored physical presence, and no contradictory South Gate card occupied either the top or center signal lanes. Evidence: `qa/collision-feedback-identity-ownership-polish-2026-08-09.md`.
- Final release verification passed 48 runtime assets, 80 deterministic action scenarios, zero source masters, zero unreferenced runtime images, and no warnings or failures.

## Simultaneous interaction ownership polish (2026-08-09)

- Audited the native East Shift Audit Bell approach and found an interaction contradiction: Audit Bell and the nearby drain valve can both be inside their true radii. The bottom rail correctly prioritized `ENTER — RING AUDIT BELL`, but the valve independently said `IN REACH` and received full ready emphasis, falsely implying that the same Enter press belonged to both objects.
- Replaced anonymous prompt text with one authoritative `interactionPromptOwner`. The selector mirrors the actual interaction priority across exclusive ball/filing modes, Emergency Appeal, Status Requests, Night Order stations, key, valve, exits, Change Requests, recoverable balls, practice, and locked exit inspection.
- Every in-reach world marker now compares itself to that owner. The owning target alone retains `IN REACH`, full ready emphasis, and the bottom-rail binding; a simultaneous non-owner says `OWNER FIRST`, drops to restrained 48% panel emphasis, loses its ready rail, and reports which target owns the action. Physical prop art and the exact interaction ring remain visible, so availability is not hidden.
- Blocked and active non-binding states retain their own local status without inventing an input. Rejection feedback also prevents unrelated in-reach markers from claiming the occupied consequence rail. The full owner object and per-marker relationship are exposed through `render_game_to_text()`.
- Added `simultaneous_interaction_prompt_ownership`, bringing readiness to 26/26, plus `web/test-actions/release-hardening-interaction-owner.json` and static verifier guards. The verifier now accepts 80 deterministic action scenarios.
- A native two-round browser route selected East Shift and reached the real overlap: Audit Bell was 10.21m inside its 12m radius; the valve was 8.06m inside its 18m radius. Audit Bell alone owned Enter and read `IN REACH`; the valve read `AUDIT BELL FIRST`, reported `other_interaction_priority`, and rendered at 48% panel emphasis. Pressing Enter filed Audit Bell, left the valve unused, and then transferred the rail to `ENTER — OPEN DRAIN VALVE`, proving clean cause → outcome → next-action handoff.
- Visual inspection passed at 2560x1600, 1280x720, and 800x600 with exact document containment and no browser errors. The official fixture-free owner replay passed 26/26 at 60 estimated FPS with 4.3ms render p95 and 31ms observed max; the native East Shift overlap pass held 60 estimated FPS with 5ms render p95 and 12.6ms observed max. Evidence: `qa/simultaneous-interaction-ownership-polish-2026-08-09.md`.

## Range-truthful objective action polish (2026-08-09)

- Audited the expanded field dossier after the Clean Break pass and found that its action row always displayed the Interact binding for the next objective, even hundreds of meters away. That falsely made distant field stations and exits look immediately usable and contradicted the grounded world markers and collision radii.
- Rebuilt the shared objective-action summary around the exact interaction target and radius. Distant targets now say `FOLLOW LANTERNS // TARGET nm`, the established approach band says `CLOSE IN`, and the configured keyboard/gamepad/touch binding appears only strictly inside the same radius used by `interactWithCourse()`. Exact-boundary contact remains approaching because the mechanic also uses a strict-inside check.
- Added dedicated `blocked`, `active filing`, `authorized`, and mandatory signal-break states. A shed/drain rejection suppresses the binding until the player retreats; filing and sealing retain their existing progress hierarchy; post-station guidance says `BREAK SIGNAL` or `HOLD QUIET` without implying another Interact press.
- Preserved the station completion handoff by explicitly resolving its `THEN` preview without the temporary signal-break override. The live dossier, pause snapshot, settings return context, and text-state diagnostics all consume the same richer action object with availability, binding ownership, exact distance, radius, approach readiness, and range rule.
- Added `objective_action_range_truth`, bringing readiness to 25/25. It checks distant, approaching, exact-boundary, and ready semantics, while the static verifier requires the new range contract and blocked-feedback ownership markers.
- Native route validation proved `FOLLOW LANTERNS // AUDIT BELL 125m`, `CLOSE IN // AUDIT BELL 13m` at 12.11m, `ENTER RING AUDIT BELL` at 9.46m, and the binding-free `BREAK SIGNAL // HEDGE TUNNEL 59m` consequence. Expanded-dossier inspection passed at 2560x1600, 1280x720, and 800x600 with exact document containment and no browser errors. The final official gameplay replay passed 25/25 at 60 estimated FPS with 4.2ms render p95, 29.2ms observed max, 79 valid action scenarios, and no errors. Evidence: `qa/range-truthful-objective-action-polish-2026-08-09.md`.

## Clean Break live-HUD continuity polish (2026-08-09)

- Audited the now-complete field/result loop and found that Clean Break mastery still disappeared during the longest part of play: after the 2.65-second reward yielded, the player had no persistent count until victory or capture. That weakened the connection between a risky station mask and the run goal it advanced.
- Added a conditional `CLEAN n/3` badge to the right side of the existing objective row in both compact and expanded HUDs. It remains completely absent at 0/3, uses restrained mint after an earned mask, turns gold at 3/3, and reserves enough width for the current objective to retain instruction priority without adding a row or permanent meter.
- `cleanBreakHudPresentation()` consumes the same authoritative unique-ID and field-only Delivery ledger used by victory and defeat. The text-state signal exposes visibility, copy, tone, placement, hierarchy, completed IDs, count, requirement, and bonus.
- Added `clean_break_live_hud_continuity`, bringing the settled readiness contract to 24/24. It rejects empty-state leakage, perfect-tone drift, incorrect copy, or displacement from the existing objective row; the static release verifier requires the same contract markers.
- The authored official breakaway route first proved the negative path: merely filing Audit Bell and later missing its cover hold kept the badge hidden. A subsequent native keyboard route followed live waypoints, sprinted the long leg, dropped to normal pace for noise decay, reached 3.06m behind `audit-arch-right`, held quiet for 0.90/0.90 seconds, recorded `signal_masked_under_named_cover`, and retained `CLEAN 1/3` after the large reward expired while Field Log resumed.
- Visual inspection passed in compact and manually expanded HUDs at 1280x720, plus the same native state at 2560x1600 and 800x600. Documents matched their viewports, the objective and badge never overlapped, the world/map/threat hierarchy remained intact, and no browser errors occurred. The final official fixture-free replay passed 24/24 at 60 estimated FPS with 3.6ms render p95, 29.4ms observed max, 79 valid action scenarios, and no errors. Evidence: `qa/clean-break-live-hud-continuity-polish-2026-08-09.md`.

## Clean Break defeat-continuity polish (2026-08-09)

- Audited failure feedback after the victory result pass and found the remaining motivation break: a player could mask one or two difficult station signals, get caught later, and see only `DELIVERY BLOCKED`. The defeat presentation erased the run's earned mastery at the moment the game asked for another attempt.
- Clean Break derivation now has one shared authoritative helper for victory, defeat, diagnostics, and field-only Delivery accounting. A capture with progress appends `CLEAN SIGNALS n/3` to the existing defeat status line; a zero-mask capture retains the original acceptance-review copy.
- The positive signal adds no new result row and does not displace Joe's expression/dialogue, cause-specific Incident Review, counterplan, or result actions. Its text-state ledger explicitly identifies this as recognition for the completed run, not permanent banking.
- A real Retry File selection after a staged two-mask capture reopened the same Night Order through the existing quick-start path, cleared the run-scoped station ledger to zero, removed capture presentation, skipped the briefing, and immediately restored control.
- Added `clean_break_defeat_continuity`, bringing the settled release contract to 23/23. It covers both perfect and empty defeat copy, placement mode, and fresh-run semantics; the static release verifier requires the same implementation markers.
- Controlled result inspection passed at 2560x1600, 1280x720, coarse-touch 844x390, and portrait-touch 390x844 with exact document containment, the expected portrait rotation prompt, and no browser errors. The staging hook and temporary scripts/artifacts were removed afterward. The final fixture-free official replay passed 23/23 at 60 estimated FPS with 4.6ms render p95, 36.8ms observed max, 79 valid action scenarios, and no browser errors. Evidence: `qa/clean-break-defeat-continuity-polish-2026-08-09.md`.

## Clean Break result continuity polish (2026-08-09)

- Audited the complete station-mastery loop and found that Clean Breaks were scored inside the generic Delivery total but disappeared as a distinct accomplishment in the After-Action Review. A successful mask therefore lacked end-of-run continuity even though the run data retained it.
- Results now derive the unique completed station IDs, `n/3` count, field-only Delivery bonus, and perfect-mask state from authoritative run records. The existing Delivery line appends `CLEAN n/3`; a perfect `3/3` receives the gold mastery accent without adding another result row or crowding the scorecard.
- The text-state result contract exposes the exact completed IDs, count, required total, field-only bonus, and perfect flag through `cleanBreakLedger`. A new readiness check rejects count, bonus, presentation, or requirement drift, bringing the live contract to 22/22.
- Native keyboard attempts organically preserved one and two completed Clean Break records through later traversal and capture, confirming that post-signal risk remains real. A controlled visual fixture used the real `awardDeliveryBeat`, `calculateRunResult`, and `completeHole` paths only to inspect a perfect `3/3` result; that temporary fixture and its window hook were removed afterward and are not release code.
- The final official settled gameplay replay passed 22/22 readiness at 60 estimated FPS with 4.1ms render p95, 30.3ms observed max, 79 valid action scenarios, and no browser errors. Perfect-result inspection passed at 2560x1600, 1280x720, and coarse-touch 390x844 with exact document containment and the portrait rotation prompt. Evidence: `qa/clean-break-result-continuity-polish-2026-08-09.md`.

## Field-signal Clean Break reward polish (2026-08-09)

- Audited the new mandatory station escape loop and found that `HOLD QUIET` was presentation-only: reaching the named cover preserved survival but provided no mechanical confirmation or positive reinforcement. It also exposed a balance defect when conservative native backpedaling could not reach the Audit Bell break point before the original signal expired.
- Every active breakaway now verifies the exact authored sight blocker, shows a live `0.0/0.9s` stillness hold, changes to `SIGNAL MASKED` when complete, and awards a capped `SIGNAL BREAKAWAY` Delivery beat only if the player remains quiet behind that cover through signal expiry. Missing the hold never blocks progression or survival.
- Signal windows now include conservative route time, noise-decay settling, the 0.9-second hold, and a 0.55-second recovery margin: Audit Bell 6.4s, Field Log 5.6s, and Release Review 5.0s. The readiness contract rejects future route/timing combinations that fall below the same bound.
- Added a dedicated amber-to-mint breakaway effect, persistent `CLEAN BREAK` reward ownership, and inspectable completion/failure reasons. Joe may still find sprint turf immediately afterward; Attention reports that consequence while the earned confirmation retains the bottom rail for 2.65 seconds.
- A native keyboard route followed live collision-aware waypoints, reached 6.71m from the Audit Bell safe point behind `audit-arch-right`, accumulated the full 0.90s quiet hold, completed `signal_masked_under_named_cover`, banked Delivery, and resumed `field-log / current_is_nearest` while Joe searched. No browser errors occurred.
- The final official activation replay passed 21/21 readiness at 60 FPS with 3.6ms render p95, 79 valid action scenarios, and no errors. Completed-reward visual inspection passed at 2560x1600, 1280x720, and coarse-touch 390x844 with exact document containment and a readable portrait rotation prompt. Evidence: `qa/field-signal-clean-break-reward-polish-2026-08-09.md`.

## Mandatory field-signal breakaway polish (2026-08-09)

- Audited the first positive objective loop and found a tactical ownership mismatch: completing a loud mandatory field check correctly sent Joe to the station, but the completion rail, route ribbon, reflectors, bearing panel, and map immediately pointed at the next distant objective. A player who trusted the UI could therefore run directly across Joe's investigation lane.
- Each of the three field checks now owns a short obstacle-aware breakaway route for the lifetime of its mandatory noise signal: Audit Bell routes behind the hedge tunnel, Field Log behind the service cart, and Release Review behind the range cart. The selected point is physically clear, reachable in 2-3 route nodes, within 50-68 meters of its station, and has its named solid cover between the station and player.
- The full handoff is explicit and stateful: completion says where to break, `TURN BACK / BREAK SIGNAL` temporarily owns the field bearing and map, the safe point has a grounded world marker, arrival changes to `SAFE BREAK REACHED / HOLD QUIET`, and signal expiry automatically replans to the next real objective. Collision recovery can still temporarily override any obstructed leg.
- A native keyboard Audit Bell route reached the guided point 5.09 meters from center, entered real hard cover behind `audit-arch-right`, preserved a blocked station sightline, survived the signal, and resumed `field-log / current_is_nearest`. Sprinting to safety still left a turf trail for Joe to investigate, preserving the speed-versus-stealth cost instead of making the maneuver free.
- Added the 20th live readiness contract, `field_signal_breakaway_routes`, plus a static verifier guard and `web/test-actions/release-hardening-field-signal-breakaway.json`. The final official replay passed 20/20 readiness at an estimated 60 FPS with 6.3 ms render p95, 79 valid action scenarios, and no browser-error artifact.
- Visual inspection passed at 2560x1600, 1280x720, and coarse-pointer 390x844 with exact viewport containment. The threat source, breakaway instruction, Joe Attention panel, map, and completion rail retained clear hierarchy; portrait retained the rotate-device prompt. Evidence: `qa/mandatory-field-signal-breakaway-polish-2026-08-09.md`.

## Collision-to-objective route handoff polish (2026-08-08)

- Audited the positive escape route and found a navigation ownership mismatch at the opening grounds cart and stone boundary: the collision card correctly said how to clear contact, but the objective ribbon and reflectors retained the pre-impact path. A player following the world route could therefore repeat the same blocked input.
- Active collision contact now converts the authoritative escape instruction into a temporary clear recovery waypoint. The world thread, reflectors, bearing panel, and text contract bend into the same lane; after contact clears, recovery ownership remains latched until the player reaches the clear waypoint or a bounded 2.4-second safety timeout expires, then the objective planner resumes from the corrected position.
- Tightened recovery completion to 0.75 meters and reserved a 0.5-meter course-edge gutter. This removes the hedge-edge oscillation where a recovery could be declared complete one input step before objective guidance turned back across the same inflated collision footprint.
- A real keyboard route activated the handoff at both opening blockers, recovered twice, reached and filed the Audit Bell, and advanced to Field Log without browser errors. Player collision, objective positions, Joe behavior, scoring, and pacing are unchanged.
- Added `collision_guidance_handoff` to live readiness and `web/test-actions/release-hardening-collision-route-handoff.json` as a staged-load-aware regression. The latest official replay returned to `current_is_nearest` with a clear Audit Bell waypoint, one retained recovery, 19/19 readiness, an estimated 60 FPS, and no browser-error artifact. The static verifier covers 78 valid action scenarios and now guards the follow-through phase plus edge gutter.
- Visual inspection passed at 2560×1600, 1280×720, and coarse-pointer 390×844. Full-page captures matched each viewport exactly, the route remained grounded below physical art, the portrait rotation prompt remained readable, and no browser error was produced. Evidence: `qa/collision-route-handoff-release-polish-2026-08-08.md`.

## Adaptive repeat-capture coaching polish (2026-08-08)

- Audited the persisted same-cause capture streak and found that repeated failures changed only the Incident Review border/title; the counterplay itself remained identical. This communicated repetition without giving a frustrated player a better experiment for the next run.
- Added bounded `guided_adjustment` and `recovery_drill` tiers for all 11 diagnosed capture causes. The second consecutive cause now asks for one concrete behavioral change; the third and later repetitions provide a short route/stealth drill. Joe speed, awareness, detection, pathing, collision, and capture thresholds are unchanged.
- Repeat state now owns the whole learning handoff: Incident Review uses `ADJUSTMENT` or `RECOVERY DRILL`, the selected Retry card uses `ADJUST` or `DRILL`, and the compact reopened run preserves the same adaptive lesson plus its mode and repeat count.
- Added `adaptive_repeat_capture_coaching` to live readiness and a corresponding static release-verifier guard. All 11 failure IDs must own both coaching tiers; current readiness is 18/18.
- Added `web/test-actions/release-hardening-repeat-capture.json`. Its uninterrupted official run completed two real Standard Review Water Hazard captures and proved `held_sightline` persisted organically to streak two. The second screen rendered `ONE CHANGE`, selected `ADJUST // BREAK SIGHT`, and replaced the base tip with a cover-to-cover correction. The 1,906-frame two-run sequence produced no browser-error artifact; exercised zone p95s remained at or below 6.0 ms.
- A controlled same-order streak-three pass verified `RECOVERY DRILL`, `DRILL // BREAK SIGHT`, and the Listening Focus/one-gap drill. Retry File reopened immediately, retained `recovery_drill / repeatCount: 3`, stayed compact, and accepted a 12-meter movement burst.
- High-resolution and responsive inspection passed at 2560x1600, 1280x720, and 390x844 with zero overflow. Both the drill defeat ledger and reopened-run drill rail remain fitted and visually subordinate to Joe/course action. Evidence: `qa/adaptive-repeat-capture-coaching-polish-2026-08-08.md`.

## Quick-retry retention-loop polish (2026-08-08)

- Extended the staged-load deep-route fixture through `Retry File` and an immediate 30-frame movement burst, creating `web/test-actions/release-hardening-deep-retry.json` as a complete defeat-to-reopened-control regression.
- The first replay confirmed that Retry already skipped Survival Briefing and the optional golf lesson, retained the exact Incident Counterplan, reset field objectives correctly, incremented the run record once, and returned to live play on the result action itself. It also exposed unnecessary visual friction: the familiar-player rematch re-expanded the full onboarding HUD for 4.2 seconds while the dedicated counterplan rail was active.
- Quick retry now uses the compact tactical HUD from its first frame. It preserves the map, route, Joe Attention, top filing banner, and bottom counterplan while removing the repeated controls/surroundings panel. Ordinary first-run onboarding, manual HUD recall, Listening Focus, and all other expansion paths are unchanged.
- Rewrote the held-sightline lesson from UI-state jargon to actionable world language: `Break sight behind solid cover, then stay quiet until Joe loses the trail.` The same truth owns Incident Review and the reopened-run counterplan.
- Added inspectable `controlRestoredImmediately` and `compact_counterplan` rematch contracts plus a static release-verifier guard. The final official replay proved actual control by moving 12 meters immediately after Retry File; it retained `first_hole`, `hudExpanded: false`, `controlHintSeconds: 0`, the exact capture-review target, current 18/18 readiness, 3.3 ms p95 canvas work, and no browser-error artifact.
- Responsive visual verification passed at 2560x1600, 1280x720, and 390x844 with zero overflow. The compact HUD and counterplan remained aligned over the live course at every size. Evidence: `qa/quick-retry-retention-loop-polish-2026-08-08.md`.

## Deep-route role continuity and defeat hierarchy polish (2026-08-08)

- Added `web/test-actions/release-hardening-deep-route.json`, a staged-load-aware deterministic route that completes the Audit Bell, follows the obstacle-aware route toward the Field Log, and exercises the investigation-to-confrontation handoff in the Water Hazard instead of stopping in the opening Tee zone.
- The authored bell noise moved Joe from patrol into investigation, raised director pressure from 0.41 to 0.99 during the subsequent chase, and produced a truthful Water Hazard incident review after the unstealthed test route was caught. This verifies that the longer route does not become an empty sprint and that defeat communicates a concrete counter-route.
- Direct screenshot review exposed one conflicting capture identity: the release-themed dialogue pack rendered `JOE // RELEASE MANAGER` beside the correct `PRODUCT OWNER: JOE` role rail. Replaced the conflicting title with the attitude `GO-LIVE CERTAIN` and added the `product_owner_role_language` readiness check plus a static release-verifier guard against Adjuster, Release Manager, Project Manager, or Scrum Master role tones.
- High-resolution review exposed duplicate default-retry guidance: Incident Review already owned the precise `NEXT RUN` counterplay while the selected Retry card repeated it as `NEXT ACTION`. The default retry state now leaves that single instruction with Incident Review; selecting Next Order or Clubhouse still renders its distinct action description.
- The final official replay reached Water Hazard defeat with the corrected `GO-LIVE CERTAIN` tone, one counterplay line, 17/17 live readiness checks, no browser-error artifact, and 3.9 ms whole-run p95 canvas work. Zone p95s were 4.0 ms in Tee, 5.7 ms in Audit Row, and 4.0 ms in Water Hazard; high-water pools remained bounded at 118/190 world particles, 31/128 screen particles, 8/18 ground responses, 2/72 world effects, and 3/3 captions.
- Responsive verification passed at 2560x1600, 1280x720, and 390x844 with zero page overflow. The 2560x1600 canvas measured 2508x1403.75; the 390x844 canvas measured 375.22x208.67. Default Retry and selected Next Order states were both visually inspected. Evidence: `qa/deep-route-product-owner-result-hierarchy-polish-2026-08-08.md`.
- Remaining acceptance is still physical: keyboard/mouse, controller, touch, and a sustained 10-15 minute mid-tier hardware session. The new per-zone telemetry should be recorded during those sessions before any broad pacing or performance retune.

## Release hardening and product-findings resolution (2026-08-08)

- Converted the remaining product findings into a bounded release-hardening pass instead of extending the game: startup delivery, runtime payload, navigation accessibility, instrumentation, metadata, and repeatable QA were the focus.
- Replaced eager image assignment with a three-phase asset coordinator: two boot images, 42 course images, and four result tableaus. The queue decodes no more than four images concurrently, exposes honest progress in the gate/menu, queues Begin until every gameplay asset settles, and records failures without deadlocking the experience.
- Moved the first expensive course composition behind the transition as a one-time prewarm. Startup and controllable gameplay now have separate measurements; text diagnostics include render and presentation average/p95/p99/max, long frames, adaptive-tier changes, load progress, and failure details.
- Reduced the deployable `web/assets` image payload from about 98.2 MB to 60.75 MB. Twenty-nine source-only, superseded, or unreferenced images and prompt records now live under `assets/source-atlases`; all 48 referenced runtime images remain present.
- Added persisted Atmosphere Density and High-Contrast Routes settings. The route option increases bypass underlay, line strength, dash frequency, and reflector opacity while preserving the authoritative route, collision, and objective geometry.
- Added a polite assistive live status region, hidden control instructions referenced by the canvas, stronger page metadata, canonical/social preview tags, critical-image preloads, and preserved keyboard focus visibility.
- Added `tools/verify-release.mjs`, `web/test-actions/release-hardening-settings.json`, and `web/test-actions/release-hardening-gameplay.json`. The verifier covers JavaScript syntax, metadata/accessibility hooks, loader/performance contracts, referenced PNG integrity, runtime payload hygiene, and every action JSON file; `--url` adds a deployed smoke test.
- Automated evidence: 48 referenced runtime images, 60.75 MB, zero missing or unreferenced runtime images, zero source masters shipped, and zero asset failures in the settled course. A post-prewarm gameplay run measured 2.89 ms average render, 4.0 ms p95, 4.8 ms max, zero long render frames, and an estimated 60 FPS.
- Visual evidence: 2560x1600 and 390x844 browser captures retained the complete centered canvas with zero body overflow. Settings retained all seven presentation controls plus the maximum-size caption preview with positive footer clearance. QA record: `qa/release-hardening-product-findings-2026-08-08.md`.
- Remaining release work is deliberately human-only: one full keyboard/mouse run, one controller run, one touch run, and a sustained mid-tier laptop session. Record repeatable findings through the new telemetry before changing collision, Joe awareness, or suspense cadence.

## Comprehensive enhancement closure (2026-08-08)

- Re-audited the 62-item product backlog against the current vertical slice. Most gameplay, tension, movement, visual, accessibility, retention, and route-fairness work was already present and inspectable; the remaining concrete engineering gaps were zone-specific performance evidence, hard caps for all high-frequency transient families, hidden-tab audio lifecycle ownership, and a callable whole-product spatial audit.
- Added per-zone render telemetry with bounded 120-sample windows. Every course zone now reports frame count, average, p95, maximum, and long-frame count, making a mid-course stall attributable instead of hiding it inside a whole-run average.
- Added explicit caps of 72 world effects and 128 screen particles, retained the existing 190 mower particles, 18 ground responses, 150 mower marks, 72 player tracks, and three captions, and exposed current plus high-water counts for those families, layered entities, atlas cells, recoverable balls, and turf marks.
- Added hidden-tab lifecycle ownership: gameplay automatically pauses, audio suspends, and explicit resume routes through the existing audio initialization gesture. Visibility pauses, audio suspends, and explicit resumes are counted in diagnostics.
- Added `window.audit_rough_cut_readiness()` and embedded its settled result in `render_game_to_text`. Eighteen live contracts cover unique/valid collision geometry, hidden collision ownership, every required target route, shed clearance, all seven footing bypasses, distinct hazard costs/art, noise-hazard art, objective art, asset settlement, map-independent navigation, Joe pressure, resource caps, frame budget, accessibility, dialogue variety, Product Owner role continuity, and adaptive repeated-failure coaching.
- The latest repeated-capture replay passed all 18 readiness checks. Its uninterrupted two-run sequence reached two Water Hazard confrontations with zero asset failures and no browser errors; Tee, Audit Row, and Water Hazard remained at or below 6.0 ms zone p95.
- High-resolution validation at 2560x1600 produced a centered 2508x1403.75 canvas and zero page overflow. Portrait validation at 390x844 retained the complete 375.22x208.67 canvas with zero overflow. The ordinary 1280x720 gameplay capture retained all world, HUD, route, map, and interaction layers without overlap.
- The complete numbered closure matrix is recorded in `qa/product-enhancement-closure-2026-08-08.md`. Software-side acceptance is automated; keyboard/mouse, controller, touch, and sustained mid-tier hardware remain physical release gates rather than unverified code claims.
...
- Replaced the procedural block-figure apparition with a dedicated ImageGen Shadow Joe sprite derived from Joe's portrait and established mower art. The versioned chroma source is archived at `assets/source-atlases/rough-cut-shadow-joe-v1-chroma.png`, while the cleaned transparent production asset remains at `web/assets/rough-cut-shadow-joe-v1.png`; both the peripheral horror event and distant spectral sightings now share the recognizable cold-eyed Joe-and-mower silhouette while retaining non-collision, flicker, dissolve, and Reduced Motion behavior.
...
- Added two dedicated built-in ImageGen horror overlays. Their original v1 masters are archived in `assets/source-atlases`, while the refined transparent v2 runtime overlays remain in `web/assets`. The spectral Joe/mower close-up and shredded moon, grass, fog, mower teeth, and flag fragments preserve the live course through authored negative space.
...
- Preserved the original chroma source and recorded the complete generation prompt, transparency settings, measured alpha bounds, and runtime roles in `assets/source-atlases/rough-cut-night-order-objectives-v1.md`.
...
- Preserved the chroma source and documented the complete prompt, exact runtime rectangle, transparency settings, and ownership contract in `assets/source-atlases/rough-cut-south-service-gate-v1.md`.
